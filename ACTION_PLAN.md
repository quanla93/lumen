...
| 7 | **i18n polish + translation docs** | 3d | [RFC 0007](docs/rfc/0007-i18n-polish.md) | Audit hardcoded EN strings; `docs/contributing/i18n.md`; lint CI for key parity | **shipped v0.7.7** — 5 surfaces promoted (settings.sso/saml/statusPage, alerts.webPush, publicStatus) + parity CI + audit script + bundle-freshness guard extended |
...
- **i18n polish (Sprint 7)** — *shipped v0.7.7*: grep hardcoded English in SSO/status page/web push panels/alerts channel hints; promote to `messages.ts`. Add namespace for dynamic pluralization ("1 host" / "5 hosts"). `docs/contributing/i18n.md`. CI script that fails when `en` and `vi` key sets diverge. **5 surfaces promoted** (settings.sso/saml/statusPage, alerts.webPush, publicStatus — 71 new keys, 615 → 686) + parity CI script (`pnpm i18n:check`) + vitest unit test + pre-PR audit script (`pnpm i18n:audit`) + bundle-freshness guard extended. **Follow-up (out of scope)**: Alerts → Maintenance tab + Settings → Backup still have inline English (~23 strings total) — same D2 recipe applies, filed for a future 1-day sweep.
