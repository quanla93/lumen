...
- [x] **v0.5.0** — completes the v1 read surface: `/api/v1/hosts/{name}`, `/api/v1/hosts/{name}/metrics?from=&to=&bucket=` (≤7d, bucket required ≥30s, cap 1000 points), `/api/v1/alerts/events?state=&limit=`, `/api/v1/alerts/rules`. Host filter + scope checks honored on every endpoint. Public Read API is now functional end-to-end.
- [ ] **v0.5.1** — docs polish: `docs/rfcs/0002-public-api.md` + `docs/src/content/docs/reference/public-api.md` (EN + VI) + Grafana JSON datasource recipe + curl examples per endpoint. Patch-level since the feature itself shipped in v0.5.0.
...
**Next**: **v0.5.1** docs polish — RFC `docs/rfcs/0002-public-api.md` + reference `docs/.../reference/public-api.md` (EN + VI) + Grafana JSON datasource recipe + curl examples. Then v0.6.0 picks up either Cold tier (if demand surfaces for >7d metrics queries) or multi-user / TOTP 2FA / Grafana spike.
