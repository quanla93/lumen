...
- [ ] `go.mod` init + dependency chọn (chi, gorilla/websocket hoặc coder/websocket, gopsutil, modernc.org/sqlite)
- [ ] `cmd/lumen-hub/main.go` — bind 8090, serve 1 endpoint `/healthz`
- [ ] `cmd/lumen-agent/main.go` — read 1 metric (CPU%), POST to hub mỗi 5s
...
**Đang làm**: Hoàn tất Phase 0 → chuẩn bị Phase 1
**Vừa hoàn thành**:
...
1. ⏳ Install Go 1.22+ (currently NOT installed on dev machine — blocker for Phase 1)
2. ⏳ Install pnpm via `npm install -g pnpm` (Node v22 đã có)
3. ⏳ `git init` + initial commit của baseline Phase 0
4. ⏳ Bắt đầu Phase 1 micro-steps:
   - 1.1 `go.mod` init + chọn deps
   - 1.2 Tạo skeleton dirs (cmd/, internal/, web/)
   - 1.3 Hub: `/healthz` endpoint
   - 1.4 Agent: collect CPU%, POST tới hub mỗi 5s
   - 1.5 Hub: WS `/api/stream`
   - 1.6 Web: Vite + 1 page hiển thị live CPU
   - 1.7 `embed.FS` web vào hub binary
   - 1.8 docker-compose dev

**Blockers / open questions**:
- ⚠️ Go chưa cài (https://go.dev/dl/)
- ⚠️ pnpm chưa cài (`npm install -g pnpm`)
