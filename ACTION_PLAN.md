...
**Next**: v0.6.2+ candidates (in rough preference order): saved views UI (schema reserved in v0.6.0, only UI missing), density toggle (`comfortable`/`compact`), host-grid virtualization (RFC 0002 N>50 cutover — defer until an operator complains). Phase 8 items: Grafana spike, public status page, first-run onboarding. Cold tier (v0.7+) still deferred behind demand signal.
