...
**Next**: nothing immediate. v0.6.1+ candidates: pull per-core CPU + Containers into the builder grid (catalog already lists them), saved views UI, density toggle, host-grid virtualization (RFC 0002 N>50 cutover). Phase 8 items: Grafana spike, public status page, first-run onboarding. Cold tier (v0.7+) still deferred behind demand signal.
