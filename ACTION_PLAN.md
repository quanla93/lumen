...
**Session**: 2026-05-29
**Đang làm**: **Phase 6 SHIPPED end-to-end — release v0.4.0.** Milestones A–D delivered (rules/channels/engine + per-rule routing + Telegram + delivery queue with severity-aware retry + Deliveries tab). Tag inventory upgraded to first-class: migration 0012 + `tags`/`tag_values` tables, new **Alerts → Tags** tab (CRUD inventory on the left, host assignment on the right; Settings → Hosts is read-only for tags now), per-key dropdowns in the rule selector picker, cascade-delete with impact dry-run + confirm. Offline-rule double-clamp fixed (`for_seconds=0` now fires at ~60s, not 120s). DoD met: `cpu_pct>90% for 5m` → ntfy/Discord/Telegram → resolve, all visible in UI. Docs synced (CHANGELOG 0.4.0, ACTION_PLAN decisions log, RFC 0001 Milestone C, `configure/alerts.md`). Go test green, web `tsc --noEmit` clean. **Deferred to a future Milestone E or v0.5**: Email (SMTP) channel; **alert/delivery retention sweep** (right now `alert_events` + `notification_deliveries` never prune — first thing to do post-release if DB grows fast); flap-suppression beyond `for_seconds`; derived/rate metrics; webhook HMAC (waits for Public API webhook unification). **Next**: deploy v0.4.0 to the user's server for real-traffic validation, then pivot to Public API / cold tier / retention sweep per priorities.
