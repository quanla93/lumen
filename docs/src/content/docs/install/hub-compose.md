...
The compose stack only includes one same-host agent. To add agents on **other** machines, use a per-agent compose file generated by the hub UI:
...
The token is shown once by the hub, then lives in the target machine's compose file. Future agent updates are standard Docker commands on that target:
