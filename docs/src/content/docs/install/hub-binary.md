...
description: Advanced/manual path for running the Lumen hub as a native Linux service without Docker.
sidebar:
  order: 1
---

This is an advanced/manual path for when you want the hub running as a regular Linux service on bare metal, a VM, or a Proxmox LXC without Docker. For the fastest default setup, use [Hub — Docker Compose](./hub-compose).
...
A hub with no agents shows nothing. The recommended agent path is a per-agent Docker Compose file generated by the hub UI:

1. Hub UI → **Settings → Hosts** → Create `my-server`.
2. Copy or download the generated `docker-compose.yml`.
3. Save it on the target machine as `/opt/lumen-agent/docker-compose.yml`.
4. Start it with `sudo docker compose up -d`.

See [Agent — Docker Compose](./agent-docker) for the full compose file and operations. If the target host cannot run Docker, use [Agent — Linux](./agent-linux) or the one-line `install.sh` shortcut as a manual/native fallback.
