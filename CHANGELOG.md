...
- **i18n polish** (Phase 8 Sprint 7, RFC 0007) — close the EN/VI drift on the five admin-only surfaces Sprint 4-6 left as inline English, and add the guard rails that make future drift mechanically catchable. See [`docs/contributing/i18n.md`](./docs/src/content/docs/contributing/i18n.md) for the contributor guide.
- **i18n follow-up sweep — Alerts → Maintenance tab + Settings → Backup** (Phase 8 Sprint 7.5, RFC 0007 §"What Sprint 7 did NOT cover"). Promotes the two follow-up surfaces Sprint 7's `i18n.md` §follow-ups flagged: `alerts.maintenance.*` (27 keys: window create form, list filter, edit/delete confirm, state badges active/upcoming/past) + `settings.backup.*` (27 keys: target select, S3 fields, passphrase, recent list, "Backup now" / "Restore" actions). Total key count: 686 → 740 (+54). Audit script reports zero inline English in those surfaces. `bundle-freshness` CI guard extended with 2 new key literals (`settings.backup.title`, `alerts.maintenance.title`). Same D2 recipe as Sprint 7 — pure copy-paste pattern.
  - **Five surfaces promoted to `messages.ts`** with EN + VI translations: `settings.sso.*` (19 keys, OIDC admin form), `settings.saml.*` (22 keys, SAML2 admin form), `settings.statusPage.*` (9 keys, public status page admin form), `alerts.webPush.*` (9 keys, browser-subscribe panel inside the channel form), `publicStatus.*` (5 keys, unauthenticated `/status` route). Total key count: 615 → 686 (+71).
  - **Parity CI guard** — `scripts/i18n-parity.mjs` (Node ESM, no AST parsing — walks the `messages.ts` tree by shape) and a vitest unit test in `web/src/i18n/messages.test.ts` (3 cases: both locales exported, same leaf-key set, no empty leaf values). Wired into CI as `pnpm i18n:check` in the `lint-web` job. The vitest is a second coverage layer on the same invariant.
  - **Pre-PR audit** — `scripts/i18n-audit.mjs` greps `web/src/**/*.{ts,tsx}` for JSX text + string-literal copy not already inside a `t("...")` call. Non-blocking (informs, doesn't gate) — the broom that should have caught Sprint 3's "source-but-not-bundle" regression at PR time, not at user-report time. Run via `pnpm i18n:audit`.
  - **`bundle-freshness` guard extended** — the CI job's grep list now also matches `settings.sso.title`, `settings.saml.title`, `settings.statusPage.title`, `alerts.webPush.subscribe`, `publicStatus.notPublished`. A future refactor that drops any of the new copy from the emitted bundle fails CI instead of silently rendering a stale UI.
  - **`tsx@^4.22.4`** added as a workspace root devDep — needed for `node --import tsx` so the parity script can import the `.ts` source directly. No runtime effect; the bundle ships without tsx.
...
- **Web Push notifications** (Phase 8 v0.7+ item). New `web_push` channel type alongside ntfy / discord / webhook / telegram / email. One channel fans out to multiple browsers — desktop Chrome/Edge/Firefox, Android, and iOS Safari (after the PWA install). VAPID key pair is generated on first use and stored in the settings table; the private key is AES-GCM-encrypted at rest with a key derived from `LUMEN_HUB_SECRET` (same scheme used for the OIDC client secret). Per-browser subscriptions live in a new `web_push_subscriptions` table (migration `0019`); 404/410 responses from the push service auto-prune dead rows. Service worker push + notificationclick handlers extend the existing `/sw.js`. Docs: [`docs/configure/web-push.md`](./docs/src/content/docs/configure/web-push.md).
  - `internal/hub/webpush` package — VAPID key gen/load (`SherClockHolmes/webpush-go` dep), subscription CRUD, send wrapper that classifies push-service errors as `IsGone()` for the dispatcher's prune path.
  - `Dispatch` signature gained a `DispatchDeps{DB, HubSecret}` parameter (was previously DB-less). All three callers — dispatcher worker, engine inline-fallback, handler `TestChannel` — updated. Backwards-compatible for the existing 5 channel types (they ignore the deps).
  - New endpoints under the session-protected group: `GET /api/alerts/web-push/vapid-public-key`, `PUT /api/alerts/web-push/subject`, `POST /api/alerts/web-push/subscribe`, `GET /api/alerts/channels/{id}/web-push/subscriptions`, `DELETE /api/alerts/web-push/subscriptions/{id}`.
  - Frontend `WebPushPanel` in the channel form renders a "Subscribe this browser" button when the channel type is `web_push`, plus an admin-visible subscription list with per-row delete.
